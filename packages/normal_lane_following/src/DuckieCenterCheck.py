...
class DetectionCheckerNode(DTROS):
    ...
    def _is_duckie_on_wrong_side(self, duckie_box):
        """Prüft ob eine Ente links von der gelben Linie ist (Gegenfahrbahn)"""
        ...
        # Suche gelbe Linie rechts von der Ente (= Ente ist auf Gegenfahrbahn)
        for m in self.yellow_masks:
            ys, xs = np.where(m > 0)
            mask_idx = (ys >= y_low) & (ys <= y_high) & (xs > duckie_box.x_max)  # rechts von Ente
            if xs[mask_idx].size > 0:
                return True  # Gelbe Linie rechts = Ente auf Gegenfahrbahn
        return False
