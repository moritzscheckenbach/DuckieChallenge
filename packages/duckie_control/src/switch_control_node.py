...
class SwitchControlNode(DTROS):
    def __init__(self, node_name):
        ...
        self.sub_intersection = rospy.Publisher(f"/{self._vehicle_name}/detect/red_stop", Bool, self.cbRedStopDetected, queue_size=1)
        self.sub_duckie = rospy.Subscriber(f"/{self._vehicle_name}/detect/duckie", Float64, self.cbDuckieDetected, queue_size=1)
        # self.sub_duckiebot = rospy.Subscriber(f"/{self._vehicle_name}/detect/duckie", Float64, self.cbDuckiebotDetected, queue_size=1)
